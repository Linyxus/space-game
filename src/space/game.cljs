...
(def simple-map {:planets [{:x 0.1, :y 0.5, :r 0.05, :m 8.5, :color [0 255 255 1]}
                           {:x 0.9, :y 0.5, :r 0.05, :m 8.5, :color [255 0 255 1]}]
...
(def simple-forcefields (generate-forcefields simple-map))
...
(defn normal-updater
  ...
  (swap! craft-status #(calc/next-state
                        {:objects %
                         :forcefields simple-forcefields}
                        (/ interval 1000))))
